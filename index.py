...
for table in alice_vars.tables.keys():
    sql_functions.create_table(
        alice_vars.db_name, table, alice_vars.tables[table])
...
def welcome(message):
    ...
    if chat_type == "private":
        ...
        user_exist = sql_functions.check_user(
            alice_vars.db_name, 'Users', chat_id)
        admin_exist = sql_functions.check_user(
            alice_vars.db_name, 'Admins', chat_id)
        if user_exist == True:
            ...
            msg = bot.send_message(
                chat_id, "Hi, " + f_name + " May I know your class?", reply_markup=alice_vars.keyboard_classes)
...
def helper(message):
    ...
    if sql_functions.check_user(alice_vars.db_name, 'Admins', chat_id):
        bot.send_message(chat_id, alice_vars.helpmsg_default + alice_vars.helpmsg_admin +
                         alice_vars.helpmsg_feedback, reply_markup=alice_vars.keyboard_admin)
    else:
        bot.send_message(chat_id, alice_vars.helpmsg_default +
                         alice_vars.helpmsg_feedback, reply_markup=alice_vars.keyboard_default)
...
def addremind(message):
    ...
    user_exist = sql_functions.check_user(
        alice_vars.db_name, 'Admins', chat_id)
    if user_exist == False:
        bot.send_message(chat_id, "Oops! You're not a bot Admin.",
...
def adddocs(message):
    ...
    if user_exist == False:
        ...
        msg = bot.send_message(chat_id, "Select the Department: ",
                               reply_markup=bot_functions.create_keyboard(alice_vars.depts))
...
def sendfeedback(message):
    ...
    if chat_id in alice_vars.superuser:
        ...
        if result == False:
            bot.send_message(chat_id, "No feedbacks recorded yet!",
                             reply_markup=alice_vars.keyboard_admin)
        else:
            feedback_msg = ""
            for entry in result:
                feedback_msg = feedback_msg+"\n" + \
                    str(entry)+": "+sql_functions.feedback_fetch(alice_vars.db_name +
                                                                 "\n\n", 'Feedback', entry)
            bot.send_message(chat_id, feedback_msg,
                             reply_markup=alice_vars.keyboard_admin)
    else:
